 ...
 * 문항에 붙기 때문이다. 텍스트 PDF는 읽은 번호를 글자로, 스캔본은 값을 읽지 않으므로
 * 그 자리를 잘라낸 그림을 그대로 보여준다. 둘 다 없으면 답만 띄운다.
 */
function PickBadge({
  region,
  label,
...
function Intro({ busy, onPick }: { busy: boolean; onPick: () => void }) {
  ...
  const notes = [
    ...
    '스캔본 — 페이지 그림에서 ①~⑤ 링과 색 번호를 직접 찾아. 쪽마다 따로 도니까 첫 쪽부터 바로 나와. 번호의 값은 읽지 않고 자리만 쓴다.',
