...
필수」로 바뀌면서 그 사유는 사라졌다.

**주의 3 — 알고 넘어가는 헌법 위반 (처리 확정).** FR-006이 동의 거부 시 앱 사용을 차단해
헌법 「데이터와 프라이버시」와 충돌한다. **헌법은 고치지 않는다** — 낮추면 출시 때 이 문제를
다시 마주칠 근거가 사라진다. 1차 MVP에서만 위반을 안고 가고, 스펙의 **「출시 전 게이트」
G-1**로 관리한다.

→ `/speckit-plan`은 이 위반을 `Complexity Tracking` 표에 반드시 기록해야 한다. 게이트를
그냥 통과시키면 안 된다.

**Clarify 세션 (2026-07-30) — 3문 3답**

| Q | 답 | 스펙 반영 |
|---|---|---|
| 동의 거부 시 무엇을 쓸 수 있나 | 앱 자체를 못 쓴다 | FR-006, US-01 시나리오 3, Out of Scope, 「헌법 정합」 |
| 신뢰도의 근거는 | AI 자기보고 확신도 | FR-004, Out of Scope 경계 명확화, Open Questions 2건 추가 |
| 정답지도 AI 단독인가 | 그렇다. 정규식은 남기되 미호출 | FR-011, Out of Scope 2행, Open Questions |

질문 없이 고친 내부 모순 셋 — 엔티티 단위(문제집당 → 페이지당), 「대기」와 「미판정」 용어 분리,
자가 검토의 낡은 수치.
