...
- **R6 판정 분리** — AI 응답에 정답/오답에 해당하는 필드가 있더라도 **무시한다.** 스키마에 그런 필드를 두지 않는다. O/X는 `detectChoice`·`gradeRegion`이 단독으로 낸다
  <!-- 근거: FR-013, 헌법 원칙 II -->
- **R7 확신도 취급** — 문항별 `confidence`(0~1)를 저장하고 화면에 드러낸다. **자동 차단선으로 쓰지 않는다** — 컷오프가 미결정이기 때문이다(Unresolved). 숫자만 보여서는 안 되고 **그 문항의 영역을 함께 보여** 학생이 눈으로 확인할 수 있어야 한다
  <!-- 근거: FR-004, 헌법 원칙 III. 근거 동반 표시의 이유: 설명 없는 확신도는 검토자를 고무도장으로 만들고, 원문 영역을 값 옆에 두면 검토가 "문서를 읽는 일"에서 "확인-또는-수정 한 동작"으로 줄어든다 — research.md D-11 -->
...
| **S-111 분석 진행** | 페이지별 진행 표시 | **단계 수** 표기 (예: "12쪽 중 3쪽") — percent-done 금지 | 분석할 페이지 없음 (전부 캐시됨) | 지연/실패 (SDD-001 E2·E3), 렌더 실패 (SDD-001 E6) |
...
**S-111에 percent-done 진행바를 쓰지 않는다.** 페이지당 지연 상한이 미결정이라(Unresolved)
정직한 퍼센트를 그릴 수 없고, 그리면 없는 숫자를 짓는 일이다 — 헌법 원칙 III와 같은 성격의
위반이다. 소요 시간을 모를 때의 권고 역시 퍼센트가 아니라 **단계 수**다(`research.md` D-11).
상한이 실측되면(quickstart V-8) 그때 다시 판단한다.

**대기 배지는 중립색을 쓴다**(S-102). 앰버·레드는 O/△/X 판정에 예약돼 있어, 대기를 경고색으로
칠하면 학생이 그것을 **판정으로 읽는다.** 논리적으로는 오채점이 아니지만 지각상으로는
오채점이고, 무너뜨리는 대상(신뢰)은 같다.
...
- ⚠️ **저신뢰 컷오프.** 확신도 몇 이하를 "미판정"으로 볼지. **알아야 할 것이 두 가지다** — (a) 골든 5권에서 확신도 ↔ 실제 오류의 상관, (b) **그 컷오프에서 페이지당 미판정이 몇 개 나오는가.** (b)를 빼면 상관이 완벽해도 제품이 안 된다: 검토량이 학생의 수용량을 넘으면 읽지 않고 넘기게 되고, SC-004는 숫자상 지켜지면서 신뢰만 무너진다(spec Open Questions · research.md D-11). 그전까지 R7대로 표시만 한다
...
|---|---|---|
| **헌법 「데이터와 프라이버시」** — *"동의를 거부해도 앱을 계속 쓸 수 있어야 한다(MUST)"* 를 FR-006이 어긴다. 동의 거부 시 앱 진입을 차단한다 | 1차 MVP의 목표가 **학습과 개발 속도**다. 거부자 전용 경로를 만들려면 기존 로컬 파이프라인(4,971줄)을 되살려 배선해야 하고, 그것이 이번 범위에서 잘라낸 바로 그 작업이다 | **대안 A(자동 채점만 비활성)** — 헌법을 지키고 개발량도 거의 0이었다(`AnswerKeyScreen.tsx` 수동 입력이 이미 있다). 사용자가 두 선택지의 차이와 미성년자 강제 동의 위험을 확인한 뒤 명시적으로 차단을 택했다. **헌법 조항은 낮추지 않았다** — 출시 전 게이트 **G-1**로 남긴다. **G-1이 요구하는 것은 "재검토"가 아니라 필수성 입증 또는 이 대안 A로의 복귀 둘 중 하나다**(spec 「출시 전 게이트」). 근거 조문은 **개인정보보호법 제16조 제3항**이며 **입증책임은 우리에게 있다**(같은 조 제1항·제22조 제3항) |
...
| 11 | 레이아웃·색이 안 들어갔는가 | ⚠️ | 화면은 "무엇이 보이는가"까지만 두되 **대기 배지의 중립색을 예외로 명시했다** — 앰버·레드가 O/△/X에 이미 배정돼 있어, 여기서 색은 미감이 아니라 **오독 방지**의 문제다. 그 외 레이아웃·색은 없다 |
| 12 | spec.md 성공 기준과 모순 없는가 | ✅ | SC-005(비행기 모드 채점) ↔ R11·E1로 성립. SC-004(오채점 0건) ↔ R6·R7 |
| 13 | 미결정이 표시됐는가 | ✅ | 5건, 각각 "무엇을 알아야 정할 수 있는지" 포함 |
| 14 | spec.md의 Open Questions를 다 처리했는가 | ✅ | 10건 중 6건을 Unresolved로 이관(신설된 「미판정 허용량」은 저신뢰 컷오프 항목의 **두 번째 축**으로 흡수했다 — 별도 항목을 만들지 않았다), 4건(대조 도입 시점·폴백 도입 시점·동의 거부 경험·신뢰도 표시)은 이번 범위 밖으로 확정 |
