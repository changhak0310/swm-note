...
학생 데이터의 영속 저장소는 기기뿐. 프록시는 중계만 하고 저장하지 않는다.
```

**아키텍처의 중심 성질은 "저장은 로컬, AI 기능은 네트워크 필수"다** (헌법 원칙 I, v3.0.0). 학생 데이터 — 문제집 PDF, 정답지, 필기 스트로크, 채점 이력, 오답노트 — 는 기기를 떠나 영속 저장되지 않는다. 반면 PDF 문항 분석은 AI에 의존하므로 네트워크 연결을 요구한다.

| | |
|---|---|
| 네트워크가 **필요한 것** | 새 문제집·정답지 페이지의 AI 분석 |
| 네트워크 **없이 동작해야 하는 것** | 필기·풀이, **이미 분석된 문항의 채점**, 오답노트 열람 |

연결이 없으면 분석은 실패가 아니라 **대기 상태**로 남고, 연결된 뒤 이어서 처리한다. 네트워크가 없다는 이유로 학생의 작업을 막아서는 안 된다.

실패 지점은 셋이다 — 파일 시스템, IndexedDB, **네트워크/프록시**.

> **v0.2까지 이 절은 "외부 의존 0"이었다.** AI 오답 원인 진단(헌법 v2.0.0)에 이어 AI PDF 분석(v3.0.0)을 채택하면서 폐기됐다. 폐기 이력과 근거는 `.specify/memory/constitution.md` 상단 Sync Impact Report에 있다.
...
평소 스택은 Next.js지만 이 프로젝트에는 맞지 않는다. Next.js는 서버 렌더링과 라우팅 서버를 전제로 설계되어 있고, Capacitor는 정적 번들을 웹뷰에 밀어 넣는 방식이다. `next export`로 우회할 수 있으나 App Router의 상당 기능이 죽고 설정이 늘어난다. 이 앱은 렌더링 서버가 없고(프록시는 API 중계일 뿐이다) 화면이 3개이므로 SSR·파일 기반 라우팅의 이득이 전혀 없다.
...
**사용하지 않는 것** — 인증 라이브러리, CAS 라이브러리, PDF 생성 라이브러리(`pdf-lib`, `jsPDF`). 1차 범위에 해당 기능이 없다.

> **이 목록에서 셋이 빠졌다.** `HTTP 클라이언트`는 프록시 호출에 필요해졌다. `OCR 엔진`은 애초에 틀린 기재였다 — `tesseract.js`를 이미 쓰고 있다(`lib/scan/ocr.ts`). `AI SDK`는 **클라이언트에는 여전히 필요 없다** — AI 호출은 프록시 서버가 하고, 앱은 프록시와만 통신한다(헌법 원칙 I: API 키 클라이언트 포함 금지).
