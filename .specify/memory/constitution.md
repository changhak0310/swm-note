...
Bump rationale: 원칙 I의 중심 주장인 "AI는 선택적 계층이다"가 사라졌다. AI가 PDF 문항 분석까지
맡으면서 채점이 AI에 의존하게 됐고, v2.0.0에서 막히던 설계가 이제 허용된다. 명확화가 아니라
후방 호환되지 않는 재정의이므로 MAJOR.
(선택지 제시 단계에서는 MINOR로 예상했으나, 원칙 제목이 바뀌는 수준이라 상향 조정했다.)

Modified principles:
  I. 로컬 우선 · AI는 선택적 계층 → I. 로컬 저장 · AI 기능은 네트워크 필수 (NON-NEGOTIABLE)
  II · III · IV — 변경 없음

Modified sections:
  데이터와 프라이버시 — 전송 목적을 "오답 원인 진단"에서 "AI 기능"으로 확대.
    동의 거부 시 무엇이 멈추는지 고지할 의무를 추가
  Governance — MAJOR 정의에 "이미 분석된 문항의 오프라인 채점 보장 제거" 추가
...
이번 개정에서 완화하지 않은 선 셋:
  1. 저장은 로컬 — 학생 데이터는 기기를 떠나 영속 저장되지 않는다
  2. 한 번 분석된 것은 오프라인 — 분석 결과가 캐시되므로 채점은 계속 오프라인 동작
  3. 필기는 언제나 가능 — 네트워크 부재가 학생의 작업을 막지 않는다

Follow-up (헌법 밖 — 이 범위가 아니므로 손대지 않았다):
  - `docs/ARCHITECTURE.md` §1 "외부 통신 없음. 서버 없음. API 키 없음."이 낡았다
  - `docs/ROADMAP.md` §0 "보류된 결정 — 오답 원인을 누가 진단하는가"가 해소됐다
  - `docs/prd/PRD-002` 보류 사유(AI 불가)가 해소됐다. 재개 가능
  - 프록시 서버는 아직 존재하지 않는다. AI 기능 전체의 선행 조건이다 (ROADMAP §11)
  - `specs/001-ai-pdf-analysis`의 「헌법 충돌」 절이 이 개정으로 해소됐다 (이번에 함께 갱신)
-->
...
### I. 로컬 저장 · AI 기능은 네트워크 필수 (NON-NEGOTIABLE)
...
**AI 호출이 필요한 기능은 네트워크 연결을 요구한다(MUST).** 연결이 없으면 그 기능은 실패가
아니라 **대기 상태**로 남아야 하고(MUST), 연결된 뒤 이어서 처리해야 한다(MUST). 네트워크가
없다는 이유로 학생의 작업(필기·풀이·문서 열람)을 막아서는 안 된다(MUST NOT).

**이미 분석이 끝난 문항의 채점과 오답노트 열람은 네트워크 없이 완전히 동작해야 한다(MUST).**
분석 결과는 기기에 저장되므로, 한 번 분석된 것은 이후 영구히 오프라인에서 쓸 수 있어야 한다.

AI 호출은 프록시 서버를 경유해야 한다(MUST). API 키를 클라이언트 번들에 포함해서는 안
된다(MUST NOT).

**근거.** v2.0.0은 AI를 오답 원인 진단에만 쓴다는 전제로 *"AI는 선택적 계층"* 이라고 규정했다.
AI가 PDF 문항 분석까지 맡으면서(`specs/001-ai-pdf-analysis`) 그 전제가 깨졌다 — 채점이 분석에
의존하고 분석이 AI에 의존한다. 사실을 문구에 반영하되 **무너지면 안 되는 선 셋은 지켰다:
저장은 로컬, 한 번 분석된 것은 오프라인, 필기는 언제나 가능.** 프록시 경유는 선택이 아니다 —
`ROADMAP.md` §11이 이미 *"배포 시 AI를 붙인다면 프록시 서버가 먼저 필요하다"* 고 적었다.

**위반 판정.** 학생 데이터를 원격에 영속 저장하거나, **이미 분석된** 문항의 채점이 네트워크를
요구하거나, 네트워크가 없다고 필기를 막거나, 클라이언트 번들에 API 키가 들어가면 위반이다.
네트워크가 없어 새 분석이 대기 상태가 되는 것은 위반이 아니다.
...
기기를 떠나는 데이터는 AI 기능 수행에 필요한 최소한이어야 하며(MUST), 무엇이 나가는지 설계
문서에 항목 단위로 열거해야 한다(MUST). "필기 전체"처럼 뭉뚱그린 서술은 허용하지 않는다.

전송 전에 학생의 명시적 동의를 받아야 한다(MUST). 동의를 거부해도 앱을 계속 쓸 수 있어야
하며(MUST), 무엇이 동작하지 않게 되는지 거부 시점에 알려야 한다(MUST). 동의는 AI 기능의
조건이지 앱 사용의 조건이 아니다.
...
- **MAJOR** — 원칙의 제거나 재정의. 학생 데이터의 원격 영속 저장 허용, AI에 채점 판정 권한을
  부여하는 변경, **이미 분석된 문항의 오프라인 채점 보장을 없애는 변경**이 여기 해당한다
